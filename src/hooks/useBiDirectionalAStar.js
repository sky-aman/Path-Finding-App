...
export default function useBiDirectionalAStar() {
	...
	const biDirectionalAStar = async () => {
		...
		try {
			...
			const startQ = new MinHeap([[0, sx, sy]]);
			const endQ = new MinHeap([[0, tx, ty]]);
				...
				for (let r of range) {
					...
					if (
						...
						if (
							...
							const newDist = startDist + manhattanDistance(target, [snx, sny]);
							startVisited[`${snx}-${sny}`] = [
								...startVisited[startKey],
								[snx, sny],
							];
							startQ.heappush([newDist, snx, sny]);
						}
					}
					if (
						enx >= 0 &&
						...
						if (
							...
							const newDist = endDist + manhattanDistance(start, [enx, eny]);
							endVisited[`${enx}-${eny}`] = [...endVisited[endKey], [enx, eny]];
							endQ.heappush([newDist, enx, eny]);
						}
					}
				}
			}
